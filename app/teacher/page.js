...
export default function TeacherPage() {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      setTimeout(() => {
        if (loading) setStatus('Claude AI is generating tough ' + examType + ' level questions... please wait');
      }, 5000);
      ...
      <div style={{ maxWidth: '800px', margin: '40px auto', padding: '0 20px' }}>
        <div style={{ background: '#fff', borderRadius: '16px', padding: '32px', border: '1px solid #e0e0e0', marginBottom: '24px' }}>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <input
              type="text"
              placeholder="e.g. Indian Polity, Organic Chemistry, Thermodynamics..."
              value={subject}
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <input
              type="range" min="5" max="30" step="1" value={numQuestions}
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <div
              style={{ border: '2px dashed #d0d0d0', borderRadius: '12px', padding: '32px', textAlign: 'center', background: file ? '#f0fff4' : '#fafafa', cursor: 'pointer', transition: 'all 0.2s' }}
            ...
            <input
              id="fileInput"
              type="file"
              accept=".pdf,.txt,.docx,.mp3,.mp4,.wav,.m4a,.webm"
              style={{ display: 'none' }}
          ...
          <div style={{ background: '#f0f4ff', borderRadius: '10px', padding: '12px 16px', marginBottom: '20px', fontSize: '13px', color: '#3355cc' }}>
            <strong>What types of questions will be generated?</strong><br />
            Analytical, reasoning-based, tricky options — {examType} level difficulty. No basic definitions.
          </div>

          {error && (
            <div style={{ background: '#fff0f0', border: '1px solid #ffcccc', borderRadius: '10px', padding: '12px 16px', marginBottom: '16px', color: '#cc0000', fontSize: '14px' }}>
              {error}
            </div>
          )}

          {status && (
            <div style={{ background: '#f0f4ff', border: '1px solid #c0ccff', borderRadius: '10px', padding: '12px 16px', marginBottom: '16px', color: '#3355cc', fontSize: '14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span>⏳</span> {status}
            </div>
          )}
          ...
          <button
            ...
            disabled={loading}
            style={{ width: '100%', padding: '15px', background: loading ? '#aaa' : '#6c63ff', color: '#fff', border: 'none', borderRadius: '12px', fontSize: '16px', fontWeight: '600', cursor: loading ? 'not-allowed' : 'pointer', transition: 'background 0.2s' }}>
          ...
          <div>
            ...
            {questions.map((q, i) => (
              <div key={i} style={{ background: '#fff', borderRadius: '14px', padding: '24px', border: '1px solid #e0e0e0', marginBottom: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                  ...
                  <span style={{
                    fontSize: '12px',
                    background: q.difficulty === 'hard' ? '#fff0f0' : q.difficulty === 'medium' ? '#fff8e1' : '#f0fff4',
                    color: q.difficulty === 'hard' ? '#c00' : q.difficulty === 'medium' ? '#aa6600' : '#2e7d32',
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '14px' }}>
                  {['A', 'B', 'C', 'D'].map(opt => (
                    <div key={opt} style={{
                      ...
                      background: q.correct === opt ? '#e8f5e9' : '#f5f5f5',
                      border: q.correct === opt ? '1.5px solid #4caf50' : '1.5px solid transparent',
            ...
            <div style={{ textAlign: 'center', marginTop: '8px', marginBottom: '40px' }}>
              <a href="/student" style={{ background: '#1a1a2e', color: '#fff', padding: '12px 32px', borderRadius: '10px', textDecoration: 'none', fontSize: '15px', fontWeight: '600' }}>
                Go to Student Page to take this quiz →
              </a>
            </div>
          </div>
        )}
